...
%   \Vdash N \in \csn$ then $\Gamma, x{:}A  \vdash M \in \csn$.
...
\begin{lemma}[Stable under Substitution of Strong Neutrals]\label{lem:SNsubst}\quad
  \begin{enumerate}
...
\begin{lemma}[$\SN$ is closed under application to a Strong Neutral]\label{lm:pSN0}
If $\Gamma \vdash M : A \arrow B \in \SN$ and $\Gamma \vdash N : A \in \SNe$ then $\Gamma \vdash M~N : B \in \SN$.
\end{lemma}
\begin{proof}
By induction on $\SN$.
...
% \begin{lemma}[Inversion Lemma about $\SN$]\label{lem:SNApp}
% If $\Gamma \vdash M~N : B \in \SN$ then 
% $\Gamma \vdash M : A \arrow B \in \SN$ and 
% $\Gamma \vdash N : A \in \SN$  
% \end{lemma}
% \begin{proof}
% By induction on the first derivation\footnote{This proof also relies on uniqueness of typing; we omit
%   this argument here as we track typing properties implicitely.}.

% \paragraph{Case} $\ianc{\Gamma \vdash M~N : B \in SNe}
%                        {\Gamma \vdash M~N : B \in \SN}{}$
% \\
% $\Gamma \vdash M : A \arrow B \in \SNe$ and 
% $\Gamma \vdash N : A \in \SN$  \hfill by inversion \\
% $\Gamma \vdash M : A \arrow B \in \SN$ \hfill by conversion rule

% \paragraph{Case} $\ibnc{\Gamma \vdash M~N \redSN M'~N : B}
%                        {\Gamma \vdash M'~N : B \in \SN}
%                        {\Gamma \vdash M~N : B \in \SN}{}$
% and $M$ is not a $\lambda$-expression
% \\
% $\Gamma \vdash M \redSN M' : A \arrow B$ \hfill by inversion on $\redSN$\\
% $\Gamma \vdash M' : A \arrow B \in \SN$ and $\Gamma \vdash N \in \SN$ \hfill by IH \\
% $\Gamma \vdash M : A \arrow B \in \SN$ \hfill by $\SN$ rule
...
% \begin{corollary}
% If   $\Gamma \vdash M \redSN M' : A$ and $\Gamma \vdash M' : A \in \SN$ and 
% $\Gamma \vdash M' : A \in \csn$
% then $\Gamma \vdash M : A \in \csn$.
% \end{corollary}
% \begin{proof}
% Direct by previous lemma \ref{lem:SNsn}
% \end{proof}

{\color{red}{
\begin{lemma}\label{lem:appsnclosureaux}
If $\Gamma, x{:}A \vdash M : B \in \csn$ and $\Gamma \vdash N : A \in \csn$
and $\Gamma \vdash [N/x]M : B \in \csn$ 
and $\Gamma, x{:}A \vdash C[x] : B' \in \csn$
and $\Gamma \vdash C[(\lambda x.M)~N] \red Q : B'$
then $\Gamma \vdash Q : B' \in \csn$
\end{lemma}}}
\begin{proof}
...
\begin{corollary}\label{lem:appsnclosure}
If $\Gamma, x{:}A \vdash M : B \in \csn$ and $\Gamma \vdash N : A \in \csn$
and $\Gamma \vdash [N/x]M : B \in \csn$ 
and $\Gamma, x{:}A \vdash C[x] : B' \in \csn$
...
\end{corollary}
\begin{proof}
Direct using the Lemma \ref{lem:appsnclosureaux}.
\end{proof}

{\color{red}{
\begin{lemma}[$\red$ and $\redSN$ are confluent]\label{lem:appsnclosurered}
If $M \red N_1$ and $M \redSN N_2$ then
\begin{itemize}
  \item Either $N_1 = N_2$
  \item Or there exists $N$ such that $N_1 \redSN N$ and $N_2 \red N$
\end{itemize}
\end{lemma}}}
\begin{proof}
By induction on both reduction proofs.
\end{proof}

{\color{red}{
\begin{lemma}[$\SN$ is closed under reduction]\label{lem:appsnclosurered}
\mbox{}
  \begin{enumerate}
  \item If $\Gamma \vdash M : A \in \SN$ and $M \red N$ then $\Gamma \vdash N : A \in \SN$
  \item If $\Gamma \vdash M : A \in \SNe$ and $M \red N$ then $\Gamma \vdash N : A \in \SNe$
  \end{enumerate}
\end{lemma}}}
\begin{proof}
By mutual structural induction on the given derivations using the fact that
$\red$ and $\redSN$ are confluent.
\end{proof}
...
$\Gamma,x{:}A \vdash C[x] : B' \in \csn$ \hfill by assumption \\
{\color{red}{$\Gamma, x{:}A \vdash C[x~N] : B' \in \csn$ \hfill by subst. property for $\csn$}}\\
